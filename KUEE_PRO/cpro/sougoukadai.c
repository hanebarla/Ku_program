...
#include "nn.h"
//#include <omp.h>
// 並列処理を行う時、6行目と185行目のコメントを外し、MinGWのpthreadの一式のパッケージをインストールして、コンパイル時には "-fopenmp", "-O3"を有効にする。
...
void int_swap(int *a, int *b); // 値交換

void shuffle(int m_x_size, int *m_x);// ランダムシャッフル

void index_init(int m_x_size, int *m_x);// 添え字初期化
...
void Relu_layer_forward(int m_x_size, float *m_x, float *output);// 順方向Relu

void Relu_layer_backward(int m_x_size, float *d_o, float *m_x, float *dm_x);// 逆方向Relu

void Softmax_layer_forward(int m_x_size, float *m_x, float *m_y);// softmax関数

void Cross_entropy_error(int m_x_size, float *m_y, int t, float *loss);// 交差エントロピー誤差

void Softmax_with_Loss_layer_backward(int m_x_size, float *m_y, float *m_t, float *dm_x);// 逆方向softmax with crossentropy error rayer

void update(int x_size, float *m_x, float *dm_x, float *adagrad); // AdaGrad

void add_slope(int m_x_size, float *m_x, float *sum_x);// 勾配の合計

void Slope_Cal(float *m_input, int label, float *W1, float *W2, float *W3, float *b1, float *b2, float *b3,
                float *d_W1, float *d_W2, float *d_W3, float *d_b1, float *d_b2, float *d_b3);
// 勾配の計算

int predict(float *m_input, float *W1, float *W2, float *W3, float *b1, float *b2, float *b3, float *loss, int label);// 推論

void save(const char *filename, int m, int n, float *A, float *b);// 保存

void load(const char *filename, int m, int n, float *A, float *b);// 読み込み

void logger(const char *filename, int gyou, float *matrix);// ロガー: csvファイルを書き出しpyplotでグラフを作成するため

void print(int size, float *matrix);// デバッグ用
...
int main(int argc, char *argv[]){
    ...
    if(argc == 4){
        ...
        float *ag_d_W1 = malloc(sizeof(float) * 39200);
        float *ag_d_b1 = malloc(sizeof(float) * 50);
        float *ag_d_W2 = malloc(sizeof(float) * 5000);
        float *ag_d_b2 = malloc(sizeof(float) * 100);
        float *ag_d_W3 = malloc(sizeof(float) * 1000);
        float *ag_d_b3 = malloc(sizeof(float) * 10);
        ...
        con_init(10, 0, d_b3);
        con_init(39200, 0, ag_d_W1);
        con_init(50, 0, ag_d_b1);
        con_init(5000, 0, ag_d_W2);
        con_init(100, 0, ag_d_b2);
        con_init(1000, 0, ag_d_W3);
        con_init(10, 0, ag_d_b3);
        ...
        for (int n = 0; n < epoch_time;n++){
            ...
            for (int i = 0; i < batch_num;i++){
                ...
                //#pragma omp parallel for //並列処理するときはこれを加える
                for (int k =i * batch_size; k < (i + 1) * batch_size; k++){

                    Slope_Cal(train_x + 784 * index[k], train_y[index[k]], W1, W2, W3, b1, b2, b3, d_W1, d_W2, d_W3, d_b1, d_b2, d_b3);
                ...
                update(39200, W1, sum_d_W1, ag_d_W1);
                update(50, b1, sum_d_b1, ag_d_b1);
                update(5000, W2, sum_d_W2, ag_d_W2);
                update(100, b2, sum_d_b2, ag_d_b2);
                update(1000, W3, sum_d_W3, ag_d_W3);
                update(10, b3, sum_d_b3, ag_d_b3);
            ...
            for (int m = 0; m < test_count;m++){
                if( predict(test_x + 784 * m, W1, W2, W3, b1, b2, b3, &ave_loss, test_y[m])==test_y[m]){
                    accuracy_conut += 1;
                    test_ac_count += 1;
                }
                // プログレスバー
            ...
            for (int o = 0; o < train_count;o++){
                if (predict(train_x + 784 * o, W1, W2, W3, b1, b2, b3, &ave_loss, train_y[o])==train_y[o]){
                    accuracy_conut += 1;
                    train_ac_count += 1;
                }
                // プログレスバー
        ...
        logger("AdaGradlog.csv", epoch_time, logging);
        ...
        load(argv[1], 784, 50, W1, b1);
        load(argv[2], 50, 100, W2, b2);
        load(argv[3], 100, 10, W3, b3);
        answer = predict(input, W1, W2, W3, b1, b2, b3, &ave_loss, label);
...
void update(int x_size, float *m_x, float *dm_x, float *adagrad){
    // SGD: m_x[i] -= L_rate * dmx[i]
    for (int i = 0; i < x_size;i++){
        dm_x[i] /= batch_size;
        adagrad[i] += dm_x[i] * dm_x[i];
    }
    for (int i = 0; i < x_size;i++){
        if(adagrad[i]>1e-12){
            m_x[i] -= L_rate * dm_x[i] / (sqrt(adagrad[i]));
        }
    }
}
...
void Slope_Cal(float *m_input, int label, float *W1, float *W2, float *W3, float *b1, float *b2, float *b3, float *d_W1, float *d_W2, float *d_W3, float *d_b1, float *d_b2, float *d_b3){
    // 活性化関数の入力部
